...
const Card = ({ children, style={} }) => <div style={{ background:"#fff", border:"1px solid #E8E6DF", borderRadius:12, padding:"16px 20px", marginBottom:12, ...style }}>{children}</div>;
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#0f2027,#203a43,#2c5364)", display:"flex", alignItems:"center", justifyContent:"center", padding:20, fontFamily:"Georgia,serif" }}>
      <div style={{ background:"#fff", borderRadius:16, padding:36, width:"100%", maxWidth:360, boxShadow:"0 24px 80px rgba(0,0,0,0.3)" }}>
        <div style={{ textAlign:"center", marginBottom:28 }}>
          <div style={{ width:52, height:52, background:"#1a1a1a", borderRadius:12, display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 12px", fontSize:16, color:"#fff", fontWeight:700 }}>MT</div>
        ...
        <div style={{ fontSize:11, color:"#bbb", textAlign:"center", marginTop:16 }}>Login: admin · Senha: admin123</div>
      </div>
    </div>
  );
};
...
const Equipe = ({ users, setUsers, onAddUser }) => {
  ...
  const [nova,setNova]=useState({nome:"",login:"",senha:"",whatsapp:"",pix:""});
  const [ci,setCi]=useState(0);
  const [saving,setSaving]=useState(false);
  const salvar=async()=>{
    if(!nova.nome||!nova.login) return;
    setSaving(true);
    const novoUser = {...nova, avatar: CORES_AVATAR[ci]};
    if(onAddUser) await onAddUser(novoUser);
    else setUsers(us=>[...us,{id:Date.now(),role:"entrevistadora",...novoUser}]);
    setNova({nome:"",login:"",senha:"",whatsapp:"",pix:""});
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
        ...
        <Btn v="green" onClick={()=>setShow(v=>!v)}>+ Nova</Btn>
      </div>
      {show&&(
        <Card>
          <ST>Cadastrar entrevistadora</ST>
      ...
      <Card>
        {users.filter(u=>u.role==="entrevistadora").length===0&&<div style={{ fontSize:13, color:"#aaa", textAlign:"center", padding:"20px 0" }}>Nenhuma entrevistadora cadastrada.</div>}
        {users.filter(u=>u.role==="entrevistadora").map(u=>(
          <div key={u.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 0", borderBottom:"1px solid #F1EFE8" }}>
            <Av nome={u.nome} bg={u.avatar?.bg} cor={u.avatar?.cor} size={36}/>
            <div style={{ flex:1 }}>
...
const buildPdfHtml = (p, fmtD) => {
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    .logo-title{font-size:36pt;font-weight:900;letter-spacing:3px;margin-bottom:8px;}
    .subtitulo{font-size:13pt;color:#555;margin-bottom:60px;}
    .cidade-title{font-size:64pt;font-weight:900;margin:30px 0;}
    .data-title{font-size:14pt;font-weight:700;margin-top:40px;}
...
const PainelAdmin = ({ user, onLogout }) => {
  ...
  const adicionarUser = async (novoUser) => {
    try {
      await sb.post("usuarios", {
        nome: novoUser.nome, login: novoUser.login, senha: novoUser.senha,
        role: "entrevistadora", avatar_bg: novoUser.avatar?.bg||"#E1F5EE",
        avatar_cor: novoUser.avatar?.cor||"#0F6E56", whatsapp: novoUser.whatsapp||"", pix: novoUser.pix||"",
      });
      await carregar();
    } catch(e) { alert("Erro ao salvar: "+e.message); }
  };
...
const AppEntrevistadora = ({ user, onLogout, pesquisas }) => {
  const minhas=pesquisas.filter(p=>p.status==="em_andamento"&&(p.entrevistadoras||[]).some(e=>e.userId===user.id));
  ...
  const enviar=()=>{setEnv(v=>[...v,{bairroId:bSel.id,resp}]);setResp({});setOk(true);setTimeout(()=>{setOk(false);setTela("bairro");},1800);};
