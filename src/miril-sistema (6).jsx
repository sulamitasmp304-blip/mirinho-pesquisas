...
const Equipe = ({ users, setUsers, onAddUser }) => {
  ...
  const [nova,setNova]=useState({nome:"",login:"",senha:"",whatsapp:"",pix:"",role:"entrevistadora"});
  ...
  const salvar=async()=>{
    ...
    setSaving(true);
    const novoUser = {...nova, nome: nova.nome.trim(), login: nova.login.trim().toLowerCase(), avatar: CORES_AVATAR[ci]};
    if(onAddUser) await onAddUser(novoUser);
    else setUsers(us=>[...us,{id:Date.now(),...novoUser}]);
    setNova({nome:"",login:"",senha:"",whatsapp:"",pix:"",role:"entrevistadora"});
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
        ...
        <Btn v="green" onClick={()=>setShow(v=>!v)}>+ Novo usuário</Btn>
      </div>
        ...
        <Card>
          ...
          <div style={{ marginBottom:16 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
              <div onClick={()=>setNova({...nova,role:"admin"})} style={{
                padding:"12px", borderRadius:10, cursor:"pointer", textAlign:"center",
                border:`2px solid ${nova.role==="admin"?"#1a1a1a":"#E8E6DF"}`,
                background:nova.role==="admin"?"#1a1a1a":"#F7F5EF",
              }}>
              ...
              <div onClick={()=>setNova({...nova,role:"entrevistadora"})} style={{
                padding:"12px", borderRadius:10, cursor:"pointer", textAlign:"center",
                border:`2px solid ${nova.role==="entrevistadora"?"#1D9E75":"#E8E6DF"}`,
                background:nova.role==="entrevistadora"?"#E1F5EE":"#F7F5EF",
              }}>
                <div style={{ fontSize:20, marginBottom:4 }}>📋</div>
      ...
      <Card>
        ...
        {entrevistadoras.map(u=>(
          <div key={u.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 0", borderBottom:"1px solid #F1EFE8" }}>
            ...
            <Tag color="green">Entrevistadora</Tag>
            <button onClick={()=>remover(u.id)} style={{ background:"transparent", border:"none", cursor:"pointer", color:"#ccc", fontSize:16, padding:"2px 6px" }}>×</button>
          </div>
        ))}
      </Card>
